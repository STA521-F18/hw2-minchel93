...
Comment: \
According to Bonferroni correction, there are no outliers in the data, and therefore no data need to be removed. As for influential points, cook's distance was absent on the residuals plot, which suggests that no data are influential points. $\\[2ex]$
...
Change: All else equal, 10\% increase in Change will increase ModernC by roghly .47.
...
PPgdp: All else equal, 10\% increase in PPgdp will increase ModernC by roghly .46.
...
Fertility: All else equal, 10\% increase in Fertility will decrease ModernC by .93. $\\[2ex]$
...
Note that $\hat{e}_{(Y)} = (I-H)Y$, $\hat{e}_{X_j} = (I-H)X_j$ and $\hat{\beta_{0}} = \vec{1}_{n X 1}\hat{\beta_{0}}$. Hereafter, assume $\vec{1}_{n X 1} = \vec{1} \\[2ex]$
